...
def _prompt_choice(message: str, options: list, default: int = 0) -> int:
    ...
    """
    default_display = default + 1  # Display as 1-based
    ...
    try:
        ...
        if not value:
            return default
        choice = int(value) - 1  # Convert back to 0-based
...
def _fetch_namespace(settings: Settings) -> Optional[str]:
    ...
    Args:
        settings: Current settings with OCI config.
    ...
    try:
        ...
        # The namespace is typically the tenancy name in lowercase
...
def _fetch_compartments(settings: Settings) -> List[tuple]:
    ...
    Returns:
        List of (name, ocid) tuples for compartments.
    """
    try:
        import oci
        ...
        return compartments[:20]  # Limit to first 20 for display
